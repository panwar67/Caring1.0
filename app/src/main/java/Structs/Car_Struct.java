package Structs;

/**
 * Created by Panwar on 28/02/17.
 */
public class Car_Struct {

    public static String Table_Name = "User_Car";
    public static String Car_Name = "CAR_NAME";
    public static String Car_Model = "CAR_MODEL";
    public static String Car_Year = "CAR_YEAR";
    public static String Car_Brand = "CAR_BRAND";
    public static String Car_FUEL = "CAR_FUEL";
    public static String Car_Segment = "CAR_SEGMENT";
    public static String Car_Code = "CAR_CODE";
    public static String Car_User = "CAR_USER";

 }
